 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Visono_Sniffs_Functions_FunctionDeclarationArgumentSpacingSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        while (($nextParam = $phpcsFile->findNext(T_VARIABLE, ($nextParam + 1), $closeBracket)) !== false) {
            ...
            if ($nextCode === T_EQUAL) {
            	...
            	//echo $nextParam."  ".$nextToken." \n";
            	//var_dump($tokens[$nextToken], $tokens[$nextParam], $tokens[$nextParam + 1]);
//             	echo strlen($tokens[$nextParam+1]['content']);

                // Check parameter default spacing.
                if (($nextToken - $nextParam) == 1 || strlen($tokens[$nextParam + 1]['content']) > 1) {

                	if (($nextToken - $nextParam) == 1) {
                		$wrong_length = 0;
                	}
                	else {
                		$wrong_length = strlen($tokens[($nextParam + 1)]['content']);
                	}
                ...
                if ($tokens[($nextToken + 1)]['content'] !== ' ') {
                	...
                	if ($tokens[($nextToken + 1)]['code'] === T_WHITESPACE) {
                		$wrong_length = strlen($tokens[($nextToken + 1)]['content']);
                	}
                	else {
                		$wrong_length = 0;
                	}
